# CIRC-Helper — older notes (BlueHive / RHEL 7.9)

These are my earlier notes, written when I was working on BlueHive (the older
RHEL 7.9 environment) for a UMAP/HDBSCAN project. I am keeping them here
because a lot of it still applies, and because some of the machines are still
running that environment.

**The main [README](README.md) is newer and written for BlueHive3**, which is
where you probably want to be. A few things below are wrong for BlueHive3 in
ways that will waste your time — most importantly the claim that Python 3.12.4
is the only version with *ssl*. That was true for the module tree on the old
BlueHive; it is not true on BlueHive3, and there is a better way to check,
which the new README explains.
